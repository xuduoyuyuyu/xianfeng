...
- Knowledge pill logo: use the Xiaowanzi-local assets `apps/wechat-miniprogram/assets/xiaowanzi-icons/knowledge-round-logo.png` and `apps/wechat-miniprogram/assets/xiaowanzi-icons/knowledge-title.png`, matching the mobile "先疯智库" entry rather than the Xiaowanzi avatar. Keep the mini-program assets in PNG format for real-device compatibility. The pill keeps the mobile 38px entry height and stays vertically centered against the WeChat capsule. Tapping it opens `https://xianfeng.xinzhi.info/experts?xw_layer=1&xw_return=xiaowanzi` through the mini-program webview.
