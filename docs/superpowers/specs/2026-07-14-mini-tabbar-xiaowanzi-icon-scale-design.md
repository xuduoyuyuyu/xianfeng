...
将微信小程序原生底栏中间的小玩子图标保持为主入口大图标，并让它的底边与其他导航图标的底边对齐，同时保留现有素材与交互。
...
- 普通图标为 `22px`，小玩子图标为 `48px`；在两者顶部起点相同的现有布局中，小玩子底边低 `26px`。仅将小玩子承载层上移 `26px`，使两类图标底边对齐。
- 不修改底栏整体高度、五等分布局、点击区域、按压动画、页面跳转和其他四个图标。
- 不替换图片素材。

## 验证

- 先更新现有静态测试，使其要求小玩子图标和承载层均为 `48px`，且小玩子承载层上移 `26px`，并确认新增断言在实现前失败。
