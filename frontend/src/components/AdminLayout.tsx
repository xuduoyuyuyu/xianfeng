...
const AdminLayout: React.FC = () => {
  ...
  const navItemClass = (active: boolean) =>
    `group flex w-full items-center justify-start gap-3 rounded-xl border px-3 py-2.5 text-left transition-all duration-200 ${
      active
  ...
  const navIconClass = (active: boolean) =>
    `material-symbols-outlined flex h-5 w-5 shrink-0 items-center justify-center text-[18px] leading-none transition-colors ${
      active ? "text-white" : "text-stone-400 group-hover:text-[#5e17eb]"
    }`;

  const navTextClass = (active: boolean) =>
    `min-w-0 text-sm leading-5 transition-colors ${active ? "font-semibold text-white" : "font-medium text-stone-600 group-hover:text-[#4e18c9]"}`;

  const renderNavItem = (to: string, icon: string, label: string, iconSrc?: string) => (
    <Link
      key={to}
      to={to}
      ...
      <aside className="w-64 bg-white border-r border-stone-200 fixed h-full">
        <div className="flex h-full flex-col">
          ...
          <nav className="flex-1 overflow-y-auto p-3 pb-4 space-y-1.5">
            ...
            <section>
              <p className="px-1 mb-1.5 text-[11px] font-bold tracking-[0.08em] text-stone-400 uppercase">决策</p>
              <div className="space-y-1">
            ...
            <section>
              ...
              <div className="space-y-1">
                ...
                {renderNavItem("/admin/mama-resources", "campaign", "妈妈好赚", "/assets/mama-hao-zhuan-icon.png")}
                {renderNavItem("/admin/welfare", "featured_seasonal_and_gifts", "小玩子百宝箱", "/assets/welfare-gift-icon.png")}
