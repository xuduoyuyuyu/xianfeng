...
const DEFAULT_CHILD_AVATAR = "/assets/wel-avatar/no-hat.png";
...
function loadChildren(): ChildProfileLite[] { try { const raw = JSON.parse(localStorage.getItem(CHILD_PROFILES_KEY)||"[]"); return Array.isArray(raw) ? raw.map((x:any)=>({ id:String(x.id||crypto.randomUUID()), relation:x.relation==="女儿"?"女儿":"儿子", displayName:String(x.displayName||""), gender:x.gender==="女"||x.relation==="女儿"?"女":"男", birthDate:String(x.birthDate||""), city:String(x.city||""), region:String(x.region||""), grade:String(x.grade||""), concernTags:Array.isArray(x.concernTags)?x.concernTags.map(String).filter(Boolean):[], avatar:String(x.avatar||DEFAULT_CHILD_AVATAR), createdAt:String(x.createdAt||new Date().toISOString()), draft:!!x.draft })) : []; } catch { return []; } }
function currentMemoryChildId(){ const children=loadChildren(); return localStorage.getItem(LAST_CHILD_ID_KEY)||children[0]?.id||DEFAULT_MEMORY_CHILD_ID; }
...
.tabs{display:flex;align-items:center;gap:8px;overflow:auto;padding:2px 0 18px}
.tab{border:1px solid #dde2eb;border-radius:999px;background:#fff;color:#101335;font-size:12px;font-weight:850;min-height:34px;padding:0 11px;display:flex;align-items:center;gap:7px;white-space:nowrap;transition:all .2s}
...
.add{width:36px;height:36px;border-radius:999px;border:1px solid #dde2eb;background:#fff;font-size:28px;color:#101335;display:flex;align-items:center;justify-content:center;flex:0 0 36px;line-height:1;font-weight:500}
...
@media(max-width:768px){
  ...
  .mobile-tab{display:grid;grid-template-columns:repeat(5,1fr);position:fixed;left:0;right:0;bottom:0;z-index:8000;background:rgba(255,255,255,.96);backdrop-filter:blur(16px);border-top:1px solid rgba(15,23,42,.08);padding:8px 10px calc(8px + env(safe-area-inset-bottom))}
...
const GlobalPublicNav: React.FC<GlobalPublicNavProps> = (props) => {
  ...
  useEffect(()=>{document.body.classList.toggle("xf-mobile-tab-enabled",compactMobile&&!embeddedLayer&&!headless); return()=>document.body.classList.remove("xf-mobile-tab-enabled")},[compactMobile,embeddedLayer,headless]);
...
function MenuPanel({setPanel,xiaowanziHomeActive}:{navLinks:React.ReactNode;setPanel:(m:PanelMode)=>void;xiaowanziHomeActive?:boolean}){
  ...
  return <>
    {isAuthed?<>
      ...
      <button className="account" onClick={openLogin}><span className="uc-av">登</span><span><strong>登录/注册</strong><small>登录后同步档案和个性化推荐</small></span><span className="chev">›</span></button>
      <div className="card">
        {isProBillingEnabled()?<MenuItem {...itemProps} to="/pro" icon="workspace_premium" label="订阅计划" active={pathname.startsWith("/pro")}/>:null}
...
function ChildPanel({back,close}:{back:()=>void;close:()=>void}){
  ...
  const [msg,setMsg]=useState("");
  const add=()=>{const n=emptyChild();const pg=parseGrade(n.grade);setActiveId(n.id);setDraft(n);setStage(pg.stage);setGradeName(pg.gradeName);setMsg("")};
  ...
  const remove=()=>{const next=items.filter(x=>x.id!==draft.id);saveChildren(next);notifyChildrenUpdated();setItems(next);const n=next[0]||emptyChild();const pg=parseGrade(n.grade);setDraft(n);setActiveId(n.id);setStage(pg.stage);setGradeName(pg.gradeName);setMsg("档案已删除")};
...
function MobileTab({embeddedLayer=false}:{embeddedLayer?:boolean}){
  ...
  return <div className="mobile-tab"><Link className={ap?"on":""} to="/programs/list"><span className="material-symbols-outlined ms">podcasts</span><span>节目</span></Link><Link className={ab?"on":""} to="/reading"><img src="/assets/jiyue-logo.png"/><span>及阅</span></Link><button className={pressing?"xw-pressing":""} onPointerDown={startPress} onPointerUp={endPress} onPointerCancel={endPress} onPointerLeave={endPress} onMouseDown={startPress} onMouseUp={endPress} onMouseLeave={endPress} onTouchStart={startPress} onTouchEnd={endPress} onClick={onClick} aria-label="小玩子，长按打开主页面"><img src={DEFAULT_CHILD_AVATAR}/></button><Link className={am?"on":""} to="/materials"><span className="material-symbols-outlined ms">inventory_2</span><span>资料</span></Link><Link className={at?"on":""} to="/topics"><span className="emoji-icon">🙏🏻</span><span>请教</span></Link></div>
}
