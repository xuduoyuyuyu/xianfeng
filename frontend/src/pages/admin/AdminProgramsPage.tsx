...
const AdminProgramsPage: React.FC = () => {
  ...
  const verificationReport = editingProgram?.agentOutputs?.enrichment?.readingVerificationReport;
                    ...
                    <div className="md:col-span-2 rounded-2xl border border-stone-200 bg-white px-4 py-4">
                      ...
                      {verificationReport ? (
                        <div className="mt-3 rounded-xl border border-sky-200 bg-sky-50/70 px-3 py-3">
                          <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
                            <div className="text-xs text-stone-700">
                              <span className="font-bold">推荐阅读校准记录：</span>
                              {verificationReport.summary || "已生成落地页校准结果。"}
                            </div>
                            <span className="rounded-full bg-white px-3 py-1 text-[11px] font-bold text-sky-700">
                              通过 {verificationReport.passedCount || 0} / {verificationReport.total || 0}
                            </span>
                          </div>
                          <details className="mt-2 text-xs text-stone-600">
                            <summary className="cursor-pointer font-bold text-sky-700">查看整页校准明细</summary>
                            <div className="mt-2 space-y-2">
                              {Array.isArray(verificationReport.items) && verificationReport.items.map((item, index) => (
                                <div
                                  key={`${item.url || item.title || "reading"}-${index}`}
                                  className={`rounded-lg border px-3 py-2 ${
                                    item.passed ? "border-emerald-200 bg-white" : "border-amber-200 bg-amber-50/60"
                                  }`}
                                >
                                  <div className="flex flex-col gap-1 md:flex-row md:items-center md:justify-between">
                                    <div className="font-bold text-stone-800">{item.title || "未命名条目"}</div>
                                    <span className={`rounded-full px-2 py-0.5 text-[10px] font-bold ${item.passed ? "bg-emerald-50 text-emerald-700" : "bg-amber-100 text-amber-700"}`}>
                                      {item.passed ? "通过" : "未通过"}
                                    </span>
                                  </div>
                                  {item.subtitle ? <div className="mt-1 text-[11px] text-stone-500">{item.subtitle}</div> : null}
                                  <div className="mt-1 text-[11px] text-stone-500">落地页：{item.landingTitle || "未解析到标题"}</div>
                                  {item.landingContributors?.length ? (
                                    <div className="mt-1 text-[11px] text-stone-500">落地页作者/译者：{item.landingContributors.join("、")}</div>
                                  ) : null}
                                  {item.issues?.length ? (
                                    <div className="mt-1 text-[11px] text-amber-700">{item.issues.join("；")}</div>
                                  ) : null}
                                </div>
                              ))}
                            </div>
