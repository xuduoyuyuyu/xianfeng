...
describe("xiaowanziContextService", () => {
  ...
  it("formats site cards as concise prompt references", () => {
    ...
    assert.match(block, /\[1\] 学习资料: \[升学规划表\]\(\/materials\/123\?xw_layer=1&xw_return=xiaowanzi\)/);
    assert.match(block, /分阶段列出准备事项。/);
    assert.match(block, /\[升学规划表\]\(\/materials\/123\?xw_layer=1&xw_return=xiaowanzi\)/);
    assert.match(block, /不要单独输出 URL/);
    assert.match(block, /同一个标题在同一次回复中只加一次链接/);
