...
exports.titleCaseEnglishToken = titleCaseEnglishToken;
exports.buildGlossaryDefinition = buildGlossaryDefinition;
exports.extractCandidateTerms = extractCandidateTerms;
exports.recoverRunningAgentTasks = recoverRunningAgentTasks;
exports.startAgentTaskDispatcher = startAgentTaskDispatcher;
exports.stopAgentTaskDispatcher = stopAgentTaskDispatcher;
exports.validateTaskTarget = validateTaskTarget;
exports.createAgentTask = createAgentTask;
exports.retryAgentTask = retryAgentTask;
exports.normalizeTaskResponse = normalizeTaskResponse;
...
const educationDictionary_1 = require("./educationDictionary");
const adminInbox_1 = require("./adminInbox");
const agentModelRegistry_1 = require("./agentModelRegistry");
const POLL_MS = 1500;
let pollTimer = null;
let working = false;
...
function getLatestPromptDoc(bucket) {
...
function resolveTaskConfig(taskType) {
    const agentCode = TASK_AGENT_MAP[taskType] || "";
    const store = (0, agentModelRegistry_1.ensureStore)(() => ({ agents: [], prompts: {}, policies: {}, strategies: {}, runs: [] }));
    const agent = store.agents.find((x) => x.agent_code === agentCode) || null;
    const bucket = agentCode ? store.prompts?.[agentCode] : null;
    const promptDoc = getLatestPromptDoc(bucket);
    const model = agent ? (0, agentModelRegistry_1.resolveAgentModelConfig)(agent, store.model_registry || []).primary : null;
    return {
        agentCode,
        promptDoc,
        model,
...
function titleCaseEnglishToken(text) {
    return text.replace(/\b(ai|gpt|llm)\b/gi, (token) => token.toUpperCase());
}
function looksLikeReferenceUrl(url) {
    return /^https?:\/\//i.test(url);
}
function buildGlossaryDefinition(term) {
    return `${term}：节目中提及的关键概念，建议结合上下文进一步核验并补充权威定义。`;
}
function termSourceUrl(term) {
...
function isNoisyReadingTitle(title) {
    const t = asText(title);
    if (!t)
        return true;
    return /(欢迎来到|最新一期|阶段的教育|因为从政策|意味着这个教育|JESSIE|^延伸阅读：)/i.test(t);
}
function isGeneratedFallbackReading(item) {
    const title = asText(item?.title);
    const subtitle = asText(item?.subtitle);
    const url = asText(item?.url);
    if (isSearchEntryUrl(url) || isSearchLikeUrl(url))
        return true;
    if (/^延伸阅读：/.test(title))
        return true;
    if (subtitle === "概念词条与背景知识" || subtitle === "概念入门与背景梳理")
        return true;
    if (url.includes("baike.baidu.com/item/") && /^延伸阅读：/.test(title))
        return true;
    return false;
}
...
function decodeHtmlContent(text) {
...
function isRealReferenceUrl(url) {
...
function inferSocialPlatform(url) {
    const value = asText(url).toLowerCase();
    if (value.includes("weibo.com"))
        return "微博";
    if (value.includes("xiaohongshu.com"))
        return "小红书";
    if (value.includes("mp.weixin.qq.com") || value.includes("wechat.com"))
        return "微信公众号";
    if (value.includes("zhihu.com"))
        return "知乎";
...
function inferPublicationType(materialType, url) {
    ...
    if (type.includes("论文") || link.includes("scholar") || link.includes("cnki") || link.includes("doi.org"))
        return "paper";
    if (type.includes("著作") || type.includes("图书") || type.includes("书"))
        return "book";
    if (type.includes("访谈") || type.includes("采访"))
        return "interview";
    if (type.includes("公开言论") || type.includes("公开资料") || type.includes("媒体"))
        return "media";
    return "other";
}
async function fetchReferenceDocument(url) {
    ...
    try {
        ...
        const title = decodeHtmlContent(html.match(/<meta[^>]+property=["']og:title["'][^>]+content=["']([^"]*?)["']/i)?.[1] || "") ||
            decodeHtmlContent(html.match(/<title[^>]*>([\s\S]*?)<\/title>/i)?.[1] || "");
        const description = decodeHtmlContent(html.match(/<meta[^>]+property=["']og:description["'][^>]+content=["']([^"]*?)["']/i)?.[1] || "") ||
            decodeHtmlContent(html.match(/<meta[^>]+name=["']description["'][^>]+content=["']([^"]*?)["']/i)?.[1] || "");
        const image = asText(html.match(/<meta[^>]+property=["']og:image["'][^>]+content=["']([^"]*?)["']/i)?.[1] || "") ||
...
async function fetchNewsArticlesByKeyword(keyword, limit = 2) {
...
function normalizeModelNameForUpstream(provider, modelName) {
...
function normalizeImageModelNameForUpstream(modelConfig) {
    ...
    if (provider === "doubao") {
        if (normalizedId.includes("seedream"))
            return modelId;
        const nameKey = normalizedName.toLowerCase();
        if (nameKey === "doubao-seedream-5.0")
            return "doubao-seedream-5-0-260128";
        if (nameKey === "doubao-seedream-5.0-lite")
            return "doubao-seedream-5-0-lite-250428";
    }
...
async function callAgentModelForJson(taskConfig, userPrompt) {
    const modelConfig = taskConfig.model || {};
    ...
    const systemPrompt = asText(taskConfig.promptDoc?.system_prompt) ||
...
function normalizeArtworkStyle(value) {
    void value;
    return "oriental_ink_series";
}
function styleLabel(style) {
    if (style === "oriental_ink_series")
        return "东方美学系列";
    return "东方美学系列";
}
function pickSemanticMotif(keywords) {
    const text = keywords.join(" ");
    if (/(困局|焦虑|压力|内耗|迷茫|冲突)/.test(text))
        return "张力回环";
    if (/(成长|孩子|亲子|家庭|陪伴)/.test(text))
        return "家园共振";
    if (/(方法|策略|体系|框架|模型|步骤)/.test(text))
        return "秩序骨架";
    if (/(精神|心理|情绪|安全感|自我)/.test(text))
        return "情绪涟漪";
    return "思想微光";
}
function clipByChars(value, max = 28) {
    const text = asText(value);
    if (text.length <= max)
        return text;
    return `${text.slice(0, max).trim()}...`;
}
function pickInsightLine(summary, fallback) {
...
function buildDefaultArtworkPromptPackage(input) {
    const styleDef = {
        unified: "艺术海报设计，等轴构图，东方美学，极致抽象极简红色水墨画，红黑金配色，版式高级，精准构图，大气克制，留白充足，极简主义",
        palette: "red, black, gold",
        layout: "大标题使用清晰宋体“东方美学”，主体构图稳定，留白明确，对比强烈，没有多余元素、线条或装饰噪音",
    };
    const fullPrompt = [
...
function resolveGeneratedImagePublicBaseUrl() {
...
async function persistGeneratedImage(sourceUrl) {
    ...
    const fileName = `${Date.now()}-${crypto_1.default.randomUUID().slice(0, 8)}${ext}`;
    const imageDir = path_1.default.resolve(process.cwd(), "uploads", "images");
    await promises_1.default.mkdir(imageDir, { recursive: true });
    await promises_1.default.writeFile(path_1.default.join(imageDir, fileName), Buffer.from(arrayBuffer));
    return `${resolveGeneratedImagePublicBaseUrl()}/uploads/images/${fileName}`;
}
async function callAgentImageModel(taskConfig, promptPackage) {
    const modelConfig = taskConfig.model || {};
    const apiKey = asText(modelConfig?.api_key);
...
async function collectParsedSignals(program) {
    const scoreMap = new Map();
    const push = (term, weight = 1) => {
    ...
    for (const item of dictionaryEntries)
        push(item?.term, 3);
...
function extractCandidateTerms(rawText) {
    ...
    function shouldKeepChineseToken(token) {
        ...
        if (stopwords.has(token))
            return false;
        if (fillerTokenPattern.test(token))
            return false;
        if (repeatedCharPattern.test(token))
            return false;
        if (token.length <= 2 && !knownShortEduTerms.has(token))
            return false;
        return true;
    }
    function cleanChineseToken(token) {
        if (!token)
            return "";
        const explicitMatch = token.match(/(神经可塑性|执行功能|家校协同|学习策略|专注力|双减政策|双减|元认知|成长型思维|项目式学习|差异化教学)/);
        if (explicitMatch?.[0])
    ...
    function isNoisyToken(token) {
        ...
        if (!t)
            return true;
        if (/(欢迎来到|最新一期|人间教育|因为从|意味着这个|这个教育|阶段的教育|这个阶段)/.test(t))
            return true;
        if (/^[A-Z]{2,}$/.test(t) && t.length <= 5)
            return true;
        if (/^[\u4e00-\u9fa5]{2,4}$/.test(t) && /(这个|那个|阶段|问题)/.test(t))
            return true;
        return false;
    }
    ...
    const suffixPattern = /[\u4e00-\u9fa5]{2,12}(?:能力|素养|思维|策略|课程|教育|心理|发展|训练|模型|机制|理论|方法|协同|政策|干预|评估|反馈|动机|认知|记忆|专注力|执行功能|可塑性)/g;
    ...
    for (const token of explicitMatches) {
        ...
        if (!shouldKeepChineseToken(cleanedToken))
            continue;
    ...
    for (const token of english) {
        ...
        if (englishStopwords.has(cleaned))
            continue;
        addCount(cleaned);
    }
    return Array.from(counts.entries())
        ...
        .filter((term) => (0, educationDictionary_1.isHighQualityEducationTerm)(term));
}
async function runProofreadTask(task) {
    const taskConfig = resolveTaskConfig(task.taskType);
    const program = await Program_1.default.findById(task.targetId);
    if (!program)
        throw new Error("节目不存在");
    const transcript = Array.isArray(program.transcript)
        ? program.transcript
        : [];
    ...
    const report = {
        ...
        summary: typoCount + punctuationChanges > 0
            ? `完成逐字稿校对：修正 ${typoCount} 处词形/术语，规范 ${punctuationChanges} 处标点。`
            : "未检测到明显可自动修正问题。",
    };
    await Program_1.default.findByIdAndUpdate(task.targetId, {
        $set: {
...
async function runProgramEnrichmentTask(task) {
    const taskConfig = resolveTaskConfig(task.taskType);
    const program = await Program_1.default.findById(task.targetId);
    if (!program)
        throw new Error("节目不存在");
    const forceOverwrite = task?.options?.forceOverwrite === true;
    const transcriptText = (Array.isArray(program.transcript) ? program.transcript : [])
        .map((segment) => asText(segment?.text))
        .filter(Boolean)
        .join("\n");
    const summaryText = asText(program?.summary?.body);
    const sourceText = [summaryText, transcriptText].filter(Boolean).join("\n");
    const extracted = extractCandidateTerms(sourceText);
    const existingGlossary = Array.isArray(program.termGlossary)
        ? program.termGlossary
        : [];
    const existingTermSet = new Set(existingGlossary.map((item) => normalizeTerm(item?.term)));
    ...
    const fetchedGroups = await Promise.all(readingTerms.slice(0, 4).map((term) => fetchNewsArticlesByKeyword(term, 2)));
    ...
    const currentDeepDive = program.deepDive || {};
    const existingReadings = Array.isArray(currentDeepDive?.curatedReading)
        ? currentDeepDive.curatedReading
        : [];
    const sanitizedExistingReadings = existingReadings.filter((item) => {
        const title = asText(item?.title);
        const url = asText(item?.url);
        if (!title || !url)
            return false;
        if (isSearchEntryUrl(url))
            return false;
        if (isNoisyReadingTitle(title))
            return false;
        if (isGeneratedFallbackReading(item))
            return false;
        return true;
    });
    const readingKeys = new Set(sanitizedExistingReadings.map((item) => asText(item?.title).toLowerCase()));
    ...
    await (0, educationDictionary_1.syncProgramDictionaryEntries)(String(task.targetId), nextGlossary, "ai_program");
    return {
...
async function runGuestProfileTask(task) {
    const taskConfig = resolveTaskConfig(task.taskType);
    const guest = await Guest_1.default.findById(task.targetId);
    if (!guest)
        throw new Error("嘉宾不存在");
    const overrides = task?.options?.guestProfileContext || {};
    const guestName = asText(overrides?.name) || asText(guest.name) || "嘉宾";
    const guestTitle = asText(overrides?.title) || asText(guest.title) || "特邀嘉宾";
    const guestBio = asText(overrides?.bio) || asText(guest.bio);
    const profileUrl = asText(overrides?.profileUrl) || asText(guest.profileUrl);
    const existingReferences = (Array.isArray(guest.profileReferences) ? guest.profileReferences : [])
        .map((item) => ({
    ...
    const overrideReferences = (Array.isArray(overrides?.profileReferences) ? overrides.profileReferences : [])
        ...
        .filter((item) => item.url && looksLikeReferenceUrl(item.url));
    const existingSocialProfiles = (Array.isArray(guest.socialProfiles) ? guest.socialProfiles : [])
        .map((item, index) => ({
    ...
    const overrideSocialProfiles = (Array.isArray(overrides?.socialProfiles) ? overrides.socialProfiles : [])
        ...
        .filter((item) => item.url && item.platform);
    const existingPublications = (Array.isArray(guest.publications) ? guest.publications : [])
        .map((item, index) => ({
    ...
    const overridePublications = (Array.isArray(overrides?.publications) ? overrides.publications : [])
        ...
        .filter((item) => item.title && item.url);
    const relatedPrograms = await Program_1.default.find({ "guestBindings.guestId": guest._id }, { _id: 1, title: 1, programCode: 1, summary: 1, updatedAt: 1 })
        .sort({ updatedAt: -1 })
        .limit(5)
        .lean();
    ...
    const fallbackAvatarCandidates = [
        asText(guest.avatar)
            ? { url: asText(guest.avatar), label: "当前头像", sourceUrl: asText(guest.avatar) }
            : null,
    ].filter(Boolean);
    const userPrompt = [
        `目标嘉宾：${guestName}`,
    ...
    const avatarCandidates = (Array.isArray(parsed?.avatar_candidates) ? parsed.avatar_candidates : [])
        ...
        .filter((item) => looksLikeReferenceUrl(item.url) && !isPlaceholderAvatarUrl(item.url));
    const referenceDocs = await Promise.all(nextReferences
    ...
    const enrichedAvatarCandidates = referenceDocs
        .map(({ reference, doc }) => {
        if (!looksLikeReferenceUrl(doc.image) || isPlaceholderAvatarUrl(doc.image))
            return null;
        return {
            url: doc.image,
    ...
    const nextAvatarCandidates = [...avatarCandidates, ...enrichedAvatarCandidates, ...fallbackAvatarCandidates]
        .filter((item, index, list) => {
        const url = asText(item?.url);
        if (!url || isPlaceholderAvatarUrl(url))
            return false;
    ...
    const materials = (Array.isArray(parsed?.materials) ? parsed.materials : [])
        ...
        .filter((item) => item.title && (item.source || isRealReferenceUrl(item.source_link)))
        .slice(0, 12);
    ...
    const markdown = [
        ...
        ...(nextMaterials.length
            ? nextMaterials.map((item) => `- ${item.material_type}｜${item.title}${item.source ? `（${item.source}）` : ""}${item.publish_time ? `｜${item.publish_time}` : ""}${item.source_link ? `\n  - 链接：${item.source_link}` : ""}${item.core_content ? `\n  - 摘要：${item.core_content}` : ""}`)
            : ["- 暂无可核验条目，请人工补充。"]),
        "",
        "## 资料索引",
    ...
    await Guest_1.default.findByIdAndUpdate(task.targetId, {
        $set: {
...
async function runProgramArtworkTask(task) {
    const taskConfig = resolveTaskConfig(task.taskType);
    const program = await Program_1.default.findById(task.targetId);
    if (!program)
        throw new Error("节目不存在");
    ...
    const currentCover = asText(program?.coverImage);
...
async function runTaskByType(task) {
    if (task.taskType === "proofread_transcript")
        return runProofreadTask(task);
    if (task.taskType === "enrich_program_content")
        return runProgramEnrichmentTask(task);
    if (task.taskType === "enrich_guest_profile")
        return runGuestProfileTask(task);
    if (task.taskType === "generate_program_artwork")
        return runProgramArtworkTask(task);
    throw new Error(`未知任务类型: ${task.taskType}`);
}
async function finalizeTaskSuccess(task, result) {
    const nextTask = await AgentTask_1.default.findByIdAndUpdate(task._id, {
        $set: {
            status: "succeeded",
            progress: 100,
            stage: "completed",
...
async function finalizeTaskFailure(task, message) {
    const nextTask = await AgentTask_1.default.findByIdAndUpdate(task._id, {
        $set: {
            status: "failed",
            progress: 100,
...
async function processTaskLifecycle(task) {
    try {
...
async function processOneTask() {
    ...
    working = true;
    try {
        const lockToken = crypto_1.default.randomUUID();
        const task = await AgentTask_1.default.findOneAndUpdate({ status: "queued" }, {
            $set: {
                status: "running",
                progress: 5,
                stage: "running",
...
async function recoverRunningAgentTasks() {
    await AgentTask_1.default.updateMany({ status: "running" }, {
        $set: {
            status: "queued",
            stage: "recovered",
            progress: 0,
...
async function startAgentTaskDispatcher() {
...
function stopAgentTaskDispatcher() {
    if (pollTimer)
        clearInterval(pollTimer);
    pollTimer = null;
}
async function validateTaskTarget(targetType, targetId) {
    if (!mongoose_1.default.Types.ObjectId.isValid(targetId)) {
        throw new Error("targetId 非法");
    }
    if (targetType === "program") {
        const exists = await Program_1.default.exists({ _id: new mongoose_1.default.Types.ObjectId(targetId) });
        if (!exists)
            throw new Error("目标节目不存在");
        return;
    }
    if (targetType === "guest") {
        const exists = await Guest_1.default.exists({ _id: new mongoose_1.default.Types.ObjectId(targetId) });
        if (!exists)
            throw new Error("目标嘉宾不存在");
        return;
    }
...
async function createAgentTask(input) {
    ...
    const lockToken = directRun ? crypto_1.default.randomUUID() : "";
    const created = await AgentTask_1.default.create({
    ...
    if (directRun)
        kickoffDirectTask(created);
    return created;
}
async function retryAgentTask(taskId) {
    if (!mongoose_1.default.Types.ObjectId.isValid(taskId))
        throw new Error("任务 ID 非法");
    const task = await AgentTask_1.default.findById(taskId);
    if (!task)
        throw new Error("任务不存在");
    if (task.status !== "failed" && task.status !== "canceled") {
        throw new Error("仅失败或已取消任务可重试");
    }
    const directRun = shouldRunDirectly(task.taskType);
    const lockToken = directRun ? crypto_1.default.randomUUID() : "";
    const nextTask = await AgentTask_1.default.findByIdAndUpdate(taskId, {
        $set: {
            status: directRun ? "running" : "queued",
            stage: directRun ? "running" : "queued",
            progress: directRun ? 5 : 0,
    ...
    if (directRun && nextTask)
        kickoffDirectTask(nextTask);
    return nextTask;
}
