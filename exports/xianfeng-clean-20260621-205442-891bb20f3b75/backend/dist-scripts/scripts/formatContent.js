...
async function formatContentWithAI(raw, nodeTitle, topicTitle) {
    if (!VOLC_API_KEY) {
...
请直接返回排版后的纯文本。`;
    try {
        const res = await axios_1.default.post(`${VOLC_ENDPOINT}/chat/completions`, {
...
async function main() {
    ...
    const topics = await Topic_1.default.find({ status: "published" }).lean();
    ...
    for (const topic of topics) {
        ...
        for (const key of layerKeys) {
            ...
            for (let i = 0; i < nodes.length; i++) {
                ...
                const newContent = await formatContentWithAI(node.content, node.title, topic.title);
                if (newContent !== node.content) {
                    await Topic_1.default.updateOne({ _id: topic._id, [`layers.${key}.${i}.key`]: node.key }, { $set: { [`layers.${key}.${i}.content`]: newContent } });
                    formatted++;
