...
const VOLC_ENDPOINT =
  process.env.VOLCENGINE_PUBLIC_BASE_URL ||
...
adminWorthbuyRouter.post("/", async (req: Request, res: Response) => {
  try {
    ...
    // 检查 brand 唯一键是否重复
    const existing = await WorthBuyAnalysis.findOne({ brand: brand.trim() }).lean();
    if (existing) {
...
// 管理员更新品牌分析
adminWorthbuyRouter.put("/:id", async (req: Request, res: Response) => {
  try {
    const { brand, query, status, result, submittedBy } = req.body || {};
    const update: any = {};
...
// ===== AI 生成品牌分析 =====
const worthbuyAiPrompt = (brand: string) => `你是一位资深消费分析师，专门做母婴/教育/家庭消费品的「值不值得买」分析。
...
adminWorthbuyRouter.post("/generate", async (req: Request, res: Response) => {
  try {
    ...
    // 检查是否已存在
    const existing = await WorthBuyAnalysis.findOne({ brand: brandName }).lean();
    if (existing) {
    ...
    const result = {
      ...
      alternatives: Array.isArray(aiResult.alternatives)
        ? aiResult.alternatives.slice(0, 3).map((a: any) => ({
            name: String(a.name || ""),
...
export default adminWorthbuyRouter;
