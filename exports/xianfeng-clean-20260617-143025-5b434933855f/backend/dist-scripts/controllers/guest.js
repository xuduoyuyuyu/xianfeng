...
function serializeProgramCard(program) {
    return {
...
function serializeGuestListItem(guest, programCount = 0) {
    ...
    const publications = (normalizePublications(Array.isArray(guest?.publications) ? guest.publications : []).length > 0
        ...
        : mapLegacyReferencesToPublications(profileReferences)).filter((item) => item.status === "active");
    return {
        ...
        publications,
        programCount,
        referenceCount: publications.length || profileReferences.filter((item) => asText(item?.url)).length,
    };
}
...
class GuestController {
    async getAllPublic(req, res) {
        try {
            ...
            const total = await Guest_1.default.countDocuments(filter);
            const skip = (page - 1) * pageSize;
            const guests = await Guest_1.default.find(filter)
                ...
                .limit(pageSize)
                .lean();
            const countMap = await buildGuestProgramCountMap(guests.map((item) => String(item._id)));
            res.status(200).json({
                guests: guests.map((item) => serializeGuestListItem(item, countMap.get(String(item._id)) || 0)),
                total,
                page,
                pageSize,
    ...
    async getByIdPublic(req, res) {
        try {
            ...
            const relatedPrograms = await Program_1.default.find({ "guestBindings.guestId": new mongoose_1.default.Types.ObjectId(id) }, { _id: 1, programCode: 1, title: 1, coverImage: 1, publishedAt: 1, summary: 1, description: 1 })
    ...
    async addReturnWish(req, res) {
        try {
            ...
            const guest = await Guest_1.default.findByIdAndUpdate(id, { $inc: { returnWishCount: 1 } }, { new: true });
            if (!guest) {
