...
export function normalizeAssistantLinkDisplayText(content: string): string {
  return String(content || "")
    .replace(/\r\n/g, "\n")
    .replace(/(^|\n)[ \t]*链接[：:][ \t]*(?:\n[ \t]*)?(?:https?:\/\/\S+|\/[^\s]+)/g, "$1")
    .replace(/\n{3,}/g, "\n\n")
...
function buildMaterialSearchHref(title: string): string {
...
export function buildXiaowanziMentionLinks(input: {
  ...
  (input.materials || []).forEach((material) => {
    const title = normalizeMentionTitle(material.title);
    if (!title) return;
    pushMentionLink(links, seen, material, "material", buildMaterialSearchHref(title));
  });
