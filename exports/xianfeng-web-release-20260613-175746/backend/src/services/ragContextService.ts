...
export function formatRagPromptBlock(citations: WeknoraSearchHit[]): string {
  ...
  const lines = [
    "[知识库参考资料]",
    "以下资料来自家长先疯知识库的检索结果（基于语义相似度匹配，非精确命中）。知识库片段不是前端可见的引用资料卡，不要在回答正文使用“资料1”“资料2”“资料5”等编号标注依据；如果资料提到了相关内容，可以直接用来源标题或概括为知识库检索提示。如果资料没有明确提到用户问题，请直接说明「资料中没有明确提到」，不要编造不存在的节目或内容。",
    ...usable.map((item) => {
      const title = asText(item.sourceTitle) || "知识库资料";
      const locator = asText(item.locator);
      const heading = locator ? `知识库片段: ${title} / ${locator}` : `知识库片段: ${title}`;
      return `${heading}\n${clampText(item.text)}`;
    }),
  ];
  return lines.join("\n");
