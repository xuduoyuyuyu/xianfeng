...
export function formatSiteCardsPromptBlock(cards: XiaowanziSiteCard[]): string {
  ...
  return [
    "[站内优先推荐]",
    "以下内容来自家长先疯站内结构化内容。只有下方明确列出的条目才可以作为站内节目或资源推荐；推荐时把标题直接写成 markdown 超链接，例如 [标题](/path)，不要单独输出 URL，也不要写“链接:”字段；同一个标题在同一次回复中只加一次链接。",
    "如果下方没有对应节目条目，不要说“家长先疯节目里也有聊过”或“平台里有相关专题”。",
    ...normalized.map((card, index) => `[${index + 1}] ${card.typeLabel}: [${card.title}](${card.href})\n${card.summary}`),
  ].join("\n");
}
