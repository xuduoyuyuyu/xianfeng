...
test("english word inventory includes the collected 50 words in five independent packs", () => {
  assert.equal(ENGLISH_PICTURE_NAMING_VERSION, "2026-08-14-prea1-packs-r2");
  ...
  assert.equal(ENGLISH_WORD_PACKS.every((pack) => pack.items.length === 10), true);
  const items = ENGLISH_WORD_PACKS.flatMap((pack) => pack.items);
  assert.equal(items.length, 50);
  assert.equal(new Set(items.map((item) => item.id)).size, 50);
  assert.equal(new Set(items.map((item) => item.word)).size, 50);
  assert.deepEqual(
    items.map((item) => item.word),
    [
      "cat", "dog", "bird", "fish", "duck", "horse", "cow", "sheep", "elephant", "monkey",
      "apple", "banana", "orange", "egg", "bread", "cake", "carrot", "tomato", "potato", "rice",
      "book", "pencil", "ruler", "chair", "table", "bed", "door", "window", "clock", "bag",
      "hand", "foot", "eye", "ear", "nose", "mouth", "hair", "hat", "shoe", "shirt",
      "car", "bus", "train", "bike", "boat", "plane", "truck", "sun", "tree", "flower"
    ]
  );
});
...
test("every collected word exposes one local real-photo asset", () => {
  ...
  assert.equal(new Set(items.map((item) => item.image)).size, 50);
