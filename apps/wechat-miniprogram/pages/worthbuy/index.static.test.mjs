...
test("submits once and separates auth, Pro, points, validation, and network states", () => { assert.match(js, /if \(this\._submitPromise\)/); assert.match(js, /classifyWorthBuyError/); assert.match(wxml, /<phone-login-gate[^>]*visible="\{\{false\}\}"/); assert.match(wxml, /actionErrorType === 'pro' \|\| actionErrorType === 'points'/); });
test("direct login replays the initiating analysis once", () => { assert.match(js, /_pendingWorthBuyAction = type/); assert.match(js, /const action = this\._pendingWorthBuyAction;[\s\S]*this\._pendingWorthBuyAction = ""/); assert.match(js, /if \(action === "analysis"\) this\.submitAnalysis\(\)/); });
