...
These 150 MP3 files are generated offline for the English word-reading flash test. The mini program plays them directly from the package and does not call online TTS or ASR for English pronunciation.
...
The covered-word contract is the 150-item bank in `utils/englishPictureNaming.js`. The static test verifies that every active word has a packaged MP3, a valid MP3 header, and the exact calibrated SHA-256 hash, so this set cannot drift silently from the assessment bank.
