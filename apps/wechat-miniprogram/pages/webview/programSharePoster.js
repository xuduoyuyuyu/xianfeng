...
function drawProgramSharePoster(ctx, program, qrImagePath, coverImagePath, guestAvatarImages) {
  ...
  ctx.setFillStyle("#ffffff");
  roundedRect(ctx, 28, qrPanelY, 694, 172, 28);
  ctx.drawImage(qrImagePath, 52, qrPanelY + 14, 144, 144);
  ctx.setFillStyle("#211a18");
  setFont(ctx, 29, "bold");
  ctx.fillText("微信扫码，直达本期节目", 228, qrPanelY + 53);
  ctx.setFillStyle("#817a8d");
  setFont(ctx, 22, "normal");
  ctx.fillText("打开家长先疯，继续收听与阅读", 228, qrPanelY + 94);
  ctx.fillText("长按保存，分享给更多家长", 228, qrPanelY + 134);
}
