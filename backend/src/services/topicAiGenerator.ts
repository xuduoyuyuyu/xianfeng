...
export async function generateDeepExpandContent(input: DeepExpandInput): Promise<string> {
  ...
  const prompt = `你是一位资深教育专家。请基于以下已有内容，在文末追加更深度的学术解读与解析。
...
⚠️ 重要：不要重写或覆盖已有内容。你只需要在已有内容之后，追加以下扩展部分：

## 学术视角
- 引用1-2个相关的心理学/教育学理论或研究
- 说明这些理论如何解释"${topicTitle}"中的现象
- 用家长能理解的语言解释学术概念
...
请直接返回扩展文本（不含"学术视角""深度解析"标题，用自然段落分隔）。`;
