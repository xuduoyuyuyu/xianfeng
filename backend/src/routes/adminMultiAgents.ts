...
import {
  ...
  ModelRegistryItem,
  ensureStore,
  maskModelRegistryItem,
  resolveAgentModelConfig,
  saveStore,
} from "../services/agentModelRegistry";
...
const DEFAULT_AGENTS: AgentRow[] = [
  ...
  { agent_code: "chat_manager_agent", name: "小玩子对话管理", description: "小玩子入口与用户全量上下文管理", status: "active", model_provider: "Deepseek", model_name: "deepseek-v4-flash", primary_model_id: "deepseek-v4-flash", feature_models: {}, temperature: 0.2, top_p: 0.95, max_tokens: 1200, timeout_ms: 60000, max_retry: 1, enable_web_search: false },
...
function makeDefaultPrompt(name: string) {
  return {
    id: Date.now(),
    version: "bootstrap-v1",
...
function pickAgentWritableFields(incoming: any) {
  return {
...
function makeAgentResponse(agent: AgentRow, registry: ModelRegistryItem[]) {
  return {
    ...agent,
    resolved_model_config: resolveAgentModelConfig(agent, registry),
  };
}
...
router.get("/multi-agents/:code", (req, res) => {
  ...
  if (!row) return res.status(404).json({ message: "agent 不存在" });
...
router.put(["/multi-agents/:code", "/mgmt/agents/:code"], (req, res) => {
  ...
  if (idx < 0) return res.status(404).json({ message: "agent 不存在" });
...
router.post(["/multi-agents", "/mgmt/agents"], (req, res) => {
  ...
  if (!agentCode) return res.status(400).json({ message: "agent_code 不能为空" });
  if (!/^[a-z][a-z0-9_]{2,63}$/i.test(agentCode)) return res.status(400).json({ message: "agent_code 格式错误，只允许字母数字下划线，长度 3-64" });
  if (!name) return res.status(400).json({ message: "name 不能为空" });
  if (store.agents.some((x) => x.agent_code === agentCode)) return res.status(409).json({ message: "agent_code 已存在" });
...
router.post(["/multi-agents/:code/prompts", "/mgmt/agents/:code/prompts"], (req, res) => {
  ...
  if (current) list.unshift({ ...current, is_current: false });
...
router.get(["/multi-agents/runs", "/mgmt/agents/runs"], (req, res) => {
  ...
  if (trace) rows = rows.filter((x) => String(x.trace_id || "").includes(trace));
  if (route) rows = rows.filter((x) => String(x.route_key || "") === route);
  res.json({ items: rows.slice(0, 100) });
});

router.get("/mgmt/model-registry", (_req, res) => {
  const store = ensureStore(buildSeedStore);
  res.json({ items: store.model_registry.map(maskModelRegistryItem) });
});
...
router.post("/mgmt/model-registry", (req, res) => {
  ...
  const item: ModelRegistryItem = {
    ...
    capabilities: Array.isArray(body.capabilities) ? body.capabilities.map(String) as any : [],
  ...
  saveStore(store);
  res.status(201).json({ ok: true, item: maskModelRegistryItem(item) });
});
...
router.put("/mgmt/model-registry/:id", (req, res) => {
  ...
  if (idx < 0) return res.status(404).json({ message: "模型不存在" });
  const body = req.body || {};
  ...
  store.model_registry[idx] = {
    ...
    capabilities: Array.isArray(body.capabilities) ? body.capabilities.map(String) as any : previous.capabilities,
    meta: body.meta && typeof body.meta === "object" ? body.meta : previous.meta,
    updated_at: new Date().toISOString(),
  };
  saveStore(store);
  res.json({ ok: true, item: maskModelRegistryItem(store.model_registry[idx]) });
});
