...
router.get("/system-info", auth_1.authenticate, requireAdmin_1.requireAdmin, async (_req, res) => {
    try {
        const [programs, books, materials, users] = await Promise.all([
            Program_1.default.countDocuments(),
            Book_1.default.countDocuments(),
            LearningMaterial_1.default.countDocuments(),
        ...
        const multiAgentStore = (0, agentModelRegistry_1.ensureStore)(() => ({
            agents: [],
            prompts: {},
            policies: {},
            strategies: {},
        ...
        const modelRegistrySummary = {
            ...
            byProvider: multiAgentStore.model_registry.reduce((acc, item) => {
        ...
        res.status(200).json({
            ...
            env: {
                ...
                showNotesDefaultTemplate: await (0, showNotes_1.getShowNotesDefaultTemplate)(),
                ai: {
                    ...
                    volcengine: {
                        ...
                        tosBridge: {
                            enabled: hasEnv("VOLCENGINE_TOS_ACCESS_KEY_ID") &&
...
router.get("/show-notes-template", auth_1.authenticate, requireAdmin_1.requireAdmin, async (_req, res) => {
    try {
        const template = await (0, showNotes_1.getShowNotesDefaultTemplate)();
        res.status(200).json({
            template,
            fallbackTemplate: (0, showNotes_1.getDefaultShowNotesTemplate)(),
        });
    }
    catch (error) {
...
router.put("/show-notes-template", auth_1.authenticate, requireAdmin_1.requireAdmin, async (req, res) => {
    try {
        const template = typeof req.body?.template === "string" ? req.body.template : "";
        const saved = await (0, showNotes_1.saveShowNotesDefaultTemplate)(template);
        res.status(200).json({
            template: saved,
            fallbackTemplate: (0, showNotes_1.getDefaultShowNotesTemplate)(),
        });
