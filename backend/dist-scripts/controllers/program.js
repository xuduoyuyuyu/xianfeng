...
const programAi_1 = require("../services/programAi");
const mongoose_1 = __importDefault(require("mongoose"));
const educationDictionary_1 = require("../services/educationDictionary");
const showNotes_1 = require("../services/showNotes");
const aiFailure_1 = require("../utils/aiFailure");
...
function signPreviewToken(input) {
    const payload = `${input.idOrCode}:${input.exp}`;
    return crypto_1.default.createHmac("sha256", previewSecret()).update(payload).digest("hex");
}
...
async function buildNextProgramCode() {
    const rows = await Program_1.default.find({ programCode: { $regex: /^ep\d+$/i } }, { programCode: 1 }).lean();
    let maxNo = 0;
    for (const row of rows) {
        const code = normalizeProgramCode(row?.programCode);
...
function buildPendingProgramTitle(now = new Date()) {
    const stamp = [
        now.getFullYear(),
...
function buildProgramTitleFromSourceFileName(value) {
    const raw = asText(value);
    if (!raw)
        return "";
    const parsedName = path_1.default.parse(raw).name || raw;
    const normalized = parsedName
        .replace(/^\d{10,}[-_ ]+/, "")
        .replace(/\s+/g, " ")
        .trim();
    return (0, showNotes_1.truncateByChars)(normalized, 80);
}
...
function findClockTokens(value) {
    const raw = asText(value);
    const tokens = raw.match(/\d{1,2}:\d{2}(?::\d{2})?/g);
    return Array.isArray(tokens) ? tokens : [];
}
function formatClockRange(seconds) {
...
function isTrivialTranscriptText(value) {
    ...
    if (fillers.has(compact))
        return true;
    if (compact.length <= 2 && /^[嗯啊哦呃哎诶对是好行哈]+$/u.test(compact))
        return true;
    return false;
}
function buildRangeLabel(startSec, endSec, fallbackTime) {
    if (Number.isFinite(startSec) && Number.isFinite(endSec) && endSec > startSec) {
        return `${formatClockRange(startSec)}-${formatClockRange(endSec)}`;
    }
    if (Number.isFinite(startSec)) {
        const safeEnd = Math.max(startSec + 4, Number.isFinite(endSec) ? endSec : startSec + 6);
        return `${formatClockRange(startSec)}-${formatClockRange(safeEnd)}`;
    }
...
function sanitizeTranscript(input) {
    ...
    const normalized = input
        ...
        if (!text || isTrivialTranscriptText(text))
            return null;
        ...
        let startSec = parseClockToSeconds(segment?.startSec) ??
            parseClockToSeconds(segment?.start) ??
            parseClockToSeconds(segment?.startTime) ??
            parseClockToSeconds(segment?.from);
        let endSec = parseClockToSeconds(segment?.endSec) ??
            parseClockToSeconds(segment?.end) ??
            parseClockToSeconds(segment?.endTime) ??
            parseClockToSeconds(segment?.to);
        if (startSec === null && tokens[0])
            startSec = parseClockToSeconds(tokens[0]);
        if (endSec === null && tokens[1])
            endSec = parseClockToSeconds(tokens[1]);
        if (Number.isFinite(startSec) && Number.isFinite(endSec) && endSec < startSec) {
            endSec = startSec + 4;
        }
        return {
    ...
    for (const item of normalized) {
        ...
        const canMerge = !!prev &&
            ...
            (!Number.isFinite(prev.endSec) || !Number.isFinite(item.startSec) || item.startSec - prev.endSec <= 10);
        if (!canMerge) {
            merged.push({ ...item });
            continue;
        }
        ...
        if (!Number.isFinite(prev.startSec) && Number.isFinite(item.startSec))
            prev.startSec = item.startSec;
        if (Number.isFinite(item.endSec))
    ...
    for (let i = 0; i < merged.length; i += 1) {
        ...
        if (Number.isFinite(current.startSec) && !Number.isFinite(current.endSec)) {
            const nextStart = next && Number.isFinite(next.startSec) ? next.startSec : null;
            current.endSec =
                nextStart && nextStart > current.startSec
                    ? Math.min(nextStart, current.startSec + 15)
                    : current.startSec + 6;
        }
    }
    return merged
...
function sanitizeCuratedReading(input) {
    if (!Array.isArray(input))
        return [];
    return input
        .map((item) => ({
        ...
        if (!url)
            return true;
        if (url.includes("bing.com/search") || url.includes("google.com/search") || url.includes("baidu.com/s?"))
            return false;
        if (url.includes("baike.baidu.com/item/") && /^延伸阅读：/.test(item.title))
            return false;
        return true;
    });
}
function sanitizeTermGlossary(input) {
    if (!Array.isArray(input))
        return [];
    return input
        .map((item) => ({
        ...
        aliases: Array.isArray(item?.aliases)
            ? item.aliases.map((alias) => asText(alias)).filter(Boolean).slice(0, 12)
            : [],
    }))
        .filter((item) => item.term && item.definition)
        .filter((item) => (0, educationDictionary_1.isHighQualityEducationTerm)(item.term));
}
function sanitizeQuickView(input) {
    if (!Array.isArray(input))
        return [];
    const normalized = input
        .map((item, index) => {
        ...
        if (!Number.isFinite(startSec) && Number.isFinite(endSec))
            startSec = Math.max(0, endSec - 90);
        if (!Number.isFinite(startSec))
            startSec = index * 300;
        if (!Number.isFinite(endSec) || endSec <= startSec) {
            endSec = startSec + 90;
        }
        const summary = (0, showNotes_1.truncateByChars)(item?.summary || item?.text, 300);
        if (!summary)
            return null;
        const startTime = formatClockRange(startSec);
        const endTime = formatClockRange(endSec);
        return {
            startTime,
            endTime,
...
function sanitizeShowNotesKeyMoments(input) {
    if (!Array.isArray(input))
        return [];
    return input
        .map((item) => ({
        time: asText(item?.time || item?.range),
        point: (0, showNotes_1.truncateByChars)(item?.point || item?.title || item?.summary, 120),
    }))
        .filter((item) => item.time && item.point)
        .slice(0, 20);
}
function sanitizeContentPack(input) {
    const raw = input || {};
    const quickView = sanitizeQuickView(raw?.quickView);
    const minutesText = (0, showNotes_1.truncateByChars)(raw?.minutes?.text, 1000);
    const showNotes = {
        guide: (0, showNotes_1.truncateByChars)(raw?.showNotes?.guide, 300),
        guestIntro: (0, showNotes_1.truncateByChars)(raw?.showNotes?.guestIntro, 300),
        keyMoments: sanitizeShowNotesKeyMoments(raw?.showNotes?.keyMoments),
        renderedText: asText(raw?.showNotes?.renderedText),
        templateOverride: asText(raw?.showNotes?.templateOverride),
    };
    return {
        quickView,
...
function sanitizeGuestBindings(input) {
    if (!Array.isArray(input))
        return [];
    const byGuestId = new Map();
    input.forEach((item, idx) => {
        const guestId = asObjectIdText(item?.guestId);
        if (!guestId || !mongoose_1.default.Types.ObjectId.isValid(guestId))
            return;
...
async function attachGuestBindingsToPrograms(programOrPrograms) {
    ...
    const guestIds = Array.from(new Set(normalizedRows.flatMap((row) => (Array.isArray(row?.guestBindings) ? row.guestBindings : [])
        .map((binding) => asObjectIdText(binding?.guestId))
    ...
    const guests = await Guest_1.default.find({ _id: { $in: guestIds } }).lean();
    const guestMap = new Map(guests.map((guest) => [String(guest._id), {
...
async function buildProgramResponse(program, extra = {}) {
    const attached = await (0, educationDictionary_1.attachDictionaryEntriesToPrograms)(program, false);
...
function sanitizeProgramPayload(payload, requireEpisode) {
    ...
    if (cleaned.episodes !== undefined) {
        ...
        const sanitizedEpisodes = episodes
            ...
            .filter((episode) => episode.title || episode.duration || episode.url);
        if (sanitizedEpisodes.length === 0) {
            throw new Error("请至少填写一条单集信息");
        }
        const first = sanitizedEpisodes[0];
        if (!first.title)
            throw new Error("单集标题不能为空");
        if (!first.duration)
            throw new Error("单集时长不能为空");
        if (!first.url)
...
function mergePreferManualText(manual, generated) {
    return hasText(manual) ? asText(manual) : asText(generated);
}
function mergePreferManualArray(manual, generated) {
    if (Array.isArray(manual) && manual.length > 0)
        return manual;
    return Array.isArray(generated) ? generated : [];
}
function mergeContentPack(manual, generated) {
...
function mergeAiIntoPayload(payload, generated, transcript) {
    const next = { ...payload };
...
function resolveLocalAudioPath(uploadedAudioUrl) {
    ...
    return path_1.default.join(process.cwd(), "uploads", "audio", safeName);
}
function shouldTranscodeAudioForAsr(filePath) {
    // 已移除 ffmpeg 依赖，火山引擎支持多种音频格式直接处理
    return false;
}
async function transcodeAudioToAsrMp3(filePath) {
    // 已移除 ffmpeg 依赖，此函数不再使用
    throw new Error("ffmpeg 转码功能已移除，请直接上传 mp3 格式或使用火山引擎支持的格式");
}
function ensureEpisodeFallbackOnAiFailure(payload, uploadedAudioUrl) {
...
function shouldClearAiFieldsAfterParseFailure(payload) {
    const title = asText(payload?.title);
    return title.startsWith("待解析节目-") || payload?.parseStatus === "parsing";
}
function buildParseFailurePayload(payload, uploadedAudioUrl) {
    ...
    const { summary: _summary, contentPack: _contentPack, deepDive: _deepDive, guest: _guest, termGlossary: _termGlossary, transcript: _transcript, dictionaryEntryIds: _dictionaryEntryIds, ...rest } = withEpisode;
    return {
        ...rest,
...
function ensureBaseFieldsFromGenerated(payload, generated, transcript) {
...
async function validateGuestBindingsOrThrow(bindings) {
    ...
    const guests = await Guest_1.default.find({ _id: { $in: guestIds } }, { _id: 1, status: 1 }).lean();
...
async function ensureGuestBindingsWithLazyMigration(payload, existingProgram) {
    const hasIncomingBindings = Array.isArray(payload?.guestBindings) && payload.guestBindings.length > 0;
    if (hasIncomingBindings)
        return payload;
    ...
    if (existingBindings.length > 0) {
        return {
            ...
            guestBindings: existingBindings
                ...
                .filter((item) => item.guestId && mongoose_1.default.Types.ObjectId.isValid(item.guestId)),
        };
    }
    ...
    if (!guestName || !normalizedName)
        return payload;
    let guest = await Guest_1.default.findOne({ normalizedName }).lean();
    if (!guest) {
        guest = (await Guest_1.default.create({
            name: guestName,
            normalizedName,
...
async function applyShowNotesRendering(payload, defaultTemplate) {
    const next = { ...(payload || {}) };
    ...
    const fallbackKeyMoments = quickView
        .slice(0, 8)
        .map((item) => ({ time: asText(item?.timeRangeLabel), point: (0, showNotes_1.truncateByChars)(item?.summary, 90) }))
        .filter((item) => item.time && item.point);
    ...
    const template = templateOverride || defaultTemplate || (await (0, showNotes_1.getShowNotesDefaultTemplate)());
    const renderedText = hasRenderableShowNotesContent
        ? (0, showNotes_1.renderShowNotesTemplate)({
            template,
            programTitle: asText(next?.title),
            guestName,
            guide,
            guestIntro,
            keyMomentsText: (0, showNotes_1.buildShowNotesKeyMomentsText)(keyMoments),
            minutes: minutesText,
        })
        : "";
    next.contentPack = {
        ...contentPack,
        minutes: {
            text: (0, showNotes_1.truncateByChars)(minutesText, 1000),
        },
        showNotes: {
            guide: (0, showNotes_1.truncateByChars)(guide, 300),
            guestIntro: (0, showNotes_1.truncateByChars)(guestIntro, 300),
...
const parsingProgramIds = new Set();
function getParseStaleTimeoutMs() {
    const configured = Number(process.env.PARSE_STALE_TIMEOUT_MS);
    if (Number.isFinite(configured) && configured >= 30 * 60 * 1000)
        return Math.floor(configured);
    return 90 * 60 * 1000;
}
const STALE_PARSE_ERROR_MESSAGE = "解析任务超时未完成，已自动回收，请重新解析";
function parseMetaPatch(status, message = "", progress = 0, stage = "idle") {
...
function normalizeParseFailureMessage(message) {
...
function buildProgramSourceTextForDictionary(payload) {
    const transcriptText = Array.isArray(payload?.transcript)
        ? payload.transcript.map((item) => asText(item?.text)).filter(Boolean).join("\n")
        : "";
    const title = asText(payload?.title);
    const summaryBody = asText(payload?.summary?.body);
    const quickViewText = Array.isArray(payload?.contentPack?.quickView)
        ? payload.contentPack.quickView.map((item) => asText(item?.summary)).filter(Boolean).join("\n")
        : "";
    return [title, summaryBody, quickViewText, transcriptText].filter(Boolean).join("\n");
}
function isStaleParsingProgram(program, now = Date.now()) {
    if (!program || program.parseStatus !== "parsing")
        return false;
    const id = asObjectIdText(program._id);
    if (id && parsingProgramIds.has(id))
        return false;
    const parseStartedAt = program.parseStartedAt ? new Date(program.parseStartedAt).getTime() : 0;
    if (!parseStartedAt || Number.isNaN(parseStartedAt))
        return false;
    return now - parseStartedAt >= getParseStaleTimeoutMs();
}
async function recycleStaleParsingPrograms(programs) {
    if (!Array.isArray(programs) || !programs.length)
        return;
    const now = Date.now();
    const staleIds = [];
    for (const program of programs) {
        if (!isStaleParsingProgram(program, now))
            continue;
        const id = asObjectIdText(program._id);
        if (!mongoose_1.default.Types.ObjectId.isValid(id))
            continue;
        staleIds.push(new mongoose_1.default.Types.ObjectId(id));
        Object.assign(program, parseMetaPatch("failed", STALE_PARSE_ERROR_MESSAGE, 100, "failed"));
    }
    if (!staleIds.length)
        return;
    await Program_1.default.updateMany({
        _id: { $in: staleIds },
        parseStatus: "parsing",
    }, parseMetaPatch("failed", STALE_PARSE_ERROR_MESSAGE, 100, "failed"));
}
function normalizeProgramForAiSource(program, uploadedAudioUrl, options) {
    const normalized = {
        ...program,
        autoGenerate: true,
        uploadedAudioUrl,
...
async function runAsyncParseTask(programId, uploadedAudioUrl, options) {
    if (parsingProgramIds.has(programId))
        return;
    const parseRunId = crypto_1.default.randomUUID();
    parsingProgramIds.add(programId);
    try {
        await Program_1.default.findByIdAndUpdate(programId, parseMetaPatch("parsing", "", 5, "queued"), { new: false });
        const currentProgram = await Program_1.default.findById(programId);
        if (!currentProgram) {
            parsingProgramIds.delete(programId);
            return;
        }
        const sourcePayload = normalizeProgramForAiSource(currentProgram.toObject(), uploadedAudioUrl, options);
        ...
        await Program_1.default.findByIdAndUpdate(programId, { parseProgress: 96, parseStage: "saving" }, { new: false });
        ...
        await Program_1.default.findByIdAndUpdate(programId, { ...payload, ...parsePatch }, { new: false });
        await (0, adminInbox_1.createInboxMessage)({
        ...
        await (0, educationDictionary_1.syncProgramDictionaryEntries)(programId, payload.termGlossary, "ai_program", {
            sourceText: dictionarySourceText,
        });
        if (status === "success") {
            await (0, agentTaskDispatcher_1.createAgentTask)({
        ...
        await Program_1.default.findByIdAndUpdate(programId, parseMetaPatch("failed", parseMessage, 100, "failed"), { new: false });
        await (0, adminInbox_1.createInboxMessage)({
            sourceType: "program_parse_task",
            sourceId: parseRunId,
            taskType: "program_parse",
...
function startAsyncParseTask(programId, uploadedAudioUrl, options) {
    setTimeout(() => {
        runAsyncParseTask(programId, uploadedAudioUrl, options).catch(() => { });
    }, 0);
}
async function tryAutoGenerate(payload, onProgress) {
    ...
    try {
        const provider = (0, programAi_1.resolveProgramAiProvider)();
        const transcribePath = localFilePath;
        const sourceUrlForAsr = uploadedAudioUrl; // 始终保留 sourceUrl，支持 Standard 模式
        ...
        return {
            ...
            aiStatus: "failed",
            aiMessage: (0, aiFailure_1.isTransientAiGenerationFailure)(message)
...
class ProgramController {
    async acceptProofread(req, res) {
        try {
            ...
            const corrected = program?.agentOutputs?.proofread?.correctedTranscript;
            ...
            const latest = await Program_1.default.findById(id);
            if (!latest) {
                res.status(500).json({ message: "应用成功，但读取失败" });
                return;
            }
            const attached = await (0, educationDictionary_1.attachDictionaryEntriesToPrograms)(latest, true);
    ...
    async getRelatedPublic(req, res) {
        try {
            const id = asText(req.params.id);
            const query = mongoose_1.default.Types.ObjectId.isValid(id)
                ? { _id: id, status: "published" }
                : { programCode: normalizeProgramCode(id), status: "published" };
            const current = await Program_1.default.findOne(query).lean();
            if (!current) {
                res.status(404).json({ message: "节目不存在或未上架" });
                return;
            }
            const currentGuestIds = new Set((Array.isArray(current?.guestBindings) ? current.guestBindings : [])
                .map((item) => asObjectIdText(item?.guestId))
                .filter(Boolean));
            const currentTags = new Set((Array.isArray(current?.summary?.tags) ? current.summary.tags : [])
                .map((x) => asText(x).toLowerCase())
                .filter(Boolean));
            const currentTerms = new Set((Array.isArray(current?.termGlossary) ? current.termGlossary : [])
                .map((x) => asText(x?.term).toLowerCase())
                .filter(Boolean));
            const baseFilter = { status: "published", _id: { $ne: current._id } };
            const orFilters = [];
                ...
                orFilters.push({
                    "guestBindings.guestId": {
                        $in: Array.from(currentGuestIds)
                            .map((x) => String(x))
                            .filter((x) => mongoose_1.default.Types.ObjectId.isValid(x))
            ...
            const candidatesRaw = await Program_1.default.find(filter)
                .sort({ publishedAt: -1, updatedAt: -1 })
                .limit(80)
                .lean();
            const candidates = await attachGuestBindingsToPrograms(candidatesRaw);
            const scored = candidates
                .map((item) => {
                const guestIds = new Set((Array.isArray(item?.guestBindings) ? item.guestBindings : [])
                    .map((b) => asObjectIdText(b?.guestId))
                    .filter(Boolean));
                const tagSet = new Set((Array.isArray(item?.summary?.tags) ? item.summary.tags : [])
                    .map((x) => asText(x).toLowerCase())
                    .filter(Boolean));
                const termSet = new Set((Array.isArray(item?.termGlossary) ? item.termGlossary : [])
                    .map((x) => asText(x?.term).toLowerCase())
                    .filter(Boolean));
                ...
                if (sameGuestCount > 0)
                    reasons.push("同嘉宾");
                if (tagOverlap > 0)
                    reasons.push(`同标签${tagOverlap}项`);
                if (termOverlap > 0)
                    reasons.push(`同词条${termOverlap}项`);
            ...
            res.status(200).json({
                current: {
                    ...
                    programCode: asText(current.programCode),
    ...
    async getAllPublic(req, res) {
        try {
            ...
            const total = await Program_1.default.countDocuments({ status: { $in: ["published", "group-only"] } });
            const skip = (page - 1) * pageSize;
            const programs = await Program_1.default.find({ status: { $in: ["published", "group-only"] } })
                .select({
                ...
                .limit(pageSize)
                .lean();
            const attached = await (0, educationDictionary_1.attachDictionaryEntriesToPrograms)(programs, false);
            const attachedGuests = await attachGuestBindingsToPrograms(attached);
            // 补充轻量布尔字段（transcript/deepDive 原始数据不返回，节省数百KB）
            const listWithFlags = attachedGuests.map((p) => ({
                ...p,
    ...
    async getByIdPublic(req, res) {
        try {
            ...
            let baseQuery = isObjectId
                ? { $or: [{ programCode: normalizedCode }, { _id: id }] }
                : { programCode: normalizedCode };
            const program = await Program_1.default.findOne(baseQuery);
            if (!program) {
                res.status(404).json({ message: "节目不存在或未上架" });
                return;
            }
            ...
            if (!result.guest?.name && Array.isArray(result.guestBindings)) {
                const firstGuest = result.guestBindings.find((b) => b?.guest);
                if (firstGuest?.guest) {
                    result.guest = {
        ...
        try {
            ...
            if (!shouldUsePagination) {
                const programs = await Program_1.default.find(filter)
                    ...
                    .sort({ updatedAt: -1 })
                    .lean();
                await recycleStaleParsingPrograms(programs);
                const attached = await (0, educationDictionary_1.attachDictionaryEntriesToPrograms)(programs, true);
                const attachedGuests = await attachGuestBindingsToPrograms(attached);
                const template = await (0, showNotes_1.getShowNotesDefaultTemplate)();
                res.status(200).json(await Promise.all(attachedGuests.map((item) => applyShowNotesRendering(item, template))));
                return;
            }
            ...
            const total = await Program_1.default.countDocuments(finalFilter);
            const skip = (page - 1) * pageSize;
            const programs = await Program_1.default.find(finalFilter)
                .select({
                programCode: 1, title: 1, description: 1, coverImage: 1,
                publishedAt: 1, createdAt: 1, updatedAt: 1,
                summary: 1, episodes: 1, status: 1, parseStatus: 1, parseProgress: 1, parseStage: 1,
            ...
            const attachedGuests = await attachGuestBindingsToPrograms(attached);
            const template = await (0, showNotes_1.getShowNotesDefaultTemplate)();
            const rows = await Promise.all(attachedGuests.map((item) => applyShowNotesRendering(item, template)));
            res.status(200).json({
                items: rows,
                data: rows,
                programs: rows,
    ...
    async createPreviewLink(req, res) {
        try {
            const id = asText(req.params.id);
            if (!id) {
            ...
            const query = mongoose_1.default.Types.ObjectId.isValid(id) ? { _id: id } : { programCode: normalizeProgramCode(id) };
            const program = await Program_1.default.findOne(query).lean();
            if (!program) {
                res.status(404).json({ message: "节目不存在" });
                return;
            }
            const idOrCode = asText(program.programCode) || String(program._id);
    ...
    async getByIdAdmin(req, res) {
        try {
            ...
            const attached = await (0, educationDictionary_1.attachDictionaryEntriesToPrograms)(program, true);
            const attachedGuest = await attachGuestBindingsToPrograms(attached);
            res.status(200).json(await applyShowNotesRendering(attachedGuest));
        }
        catch (error) {
    ...
    async create(req, res) {
        try {
            ...
            const program = new Program_1.default(payload);
            await program.save();
            await (0, educationDictionary_1.syncProgramDictionaryEntries)(String(program._id), payload.termGlossary, "ai_program", {
                sourceText: buildProgramSourceTextForDictionary(payload),
            });
            const latestProgram = await Program_1.default.findById(program._id);
            if (!latestProgram) {
                res.status(500).json({ message: "节目创建成功，但读取结果失败" });
                return;
            }
            res.status(201).json(await buildProgramResponse(latestProgram, {
    ...
    async update(req, res) {
        try {
            ...
            const program = await Program_1.default.findByIdAndUpdate(id, payload, { new: true });
            if (!program) {
                res.status(404).json({ message: "节目不存在" });
                return;
            }
            ...
            res.status(200).json(await buildProgramResponse(latestProgram, {
                aiStatus: aiResult.aiStatus || "skipped",
                aiMessage: aiResult.aiMessage || "",
            }));
        }
    ...
    async triggerParse(req, res) {
        try {
            ...
            if (program.parseStatus === "parsing" || parsingProgramIds.has(id)) {
                ...
                await Program_1.default.findByIdAndUpdate(id, parseMetaPatch("failed", STALE_PARSE_ERROR_MESSAGE, 100, "failed"), { new: false });
            }
    ...
    async getParseStatus(req, res) {
        try {
            ...
            res.status(200).json({
                ...
                parseStage: asText(program.parseStage) || "idle",
                parseProgress: Number(program.parseProgress) || 0,
    ...
    async updateStatus(req, res) {
        try {
            ...
            const program = await Program_1.default.findByIdAndUpdate(id, statusUpdatePayload(status), { new: true });
            if (!program) {
                res.status(404).json({ message: "节目不存在" });
                return;
            }
    ...
    async delete(req, res) {
        try {
            ...
            await (0, educationDictionary_1.removeProgramFromDictionary)(String(program._id));
