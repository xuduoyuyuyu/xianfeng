...
exports.normalizeVolcenginePublicSourceUrl = normalizeVolcenginePublicSourceUrl;
exports.shouldUseVolcengineStandardEndpoint = shouldUseVolcengineStandardEndpoint;
exports.shouldAttemptVolcengineFlashEndpoint = shouldAttemptVolcengineFlashEndpoint;
exports.getVolcengineFlashMaxLocalBytes = getVolcengineFlashMaxLocalBytes;
exports.shouldContinueVolcengineStandardPolling = shouldContinueVolcengineStandardPolling;
exports.resolveProgramAiProvider = resolveProgramAiProvider;
...
function resolveTosBridgeConfig() {
...
function guessAudioContentType(filePath) {
...
function detectAudioFormatFromUrl(sourceUrl) {
    try {
...
async function uploadLocalAudioToTosAndSign(filePath) {
    ...
    const TosClientCtor = sdk?.TosClient ||
        sdk?.TOS ||
    ...
    const objectKey = `${cfg.keyPrefix.replace(/\/+$/, "")}/${Date.now()}-${(0, crypto_1.randomUUID)()}${path_1.default.extname(filePath) || ".mp3"}`;
...
function normalizeVolcenginePublicSourceUrl(sourceUrl, publicBaseUrl) {
    const raw = asText(sourceUrl);
    const publicBase = asText(publicBaseUrl);
    if (!raw)
        return "";
    const normalizeHostForAsr = (urlText) => {
        try {
...
function shouldUseVolcengineStandardEndpoint(resourceId, mode) {
    ...
    if (normalizedMode === "standard")
        return true;
    if (/^Speech_Recognition_Seed_/i.test(normalizedResourceId))
        return true;
    if (normalizedResourceIdLower === "volc.seedasr.auc")
        return true;
    if (normalizedResourceIdLower.startsWith("volc.seedasr."))
        return true;
    return normalizedResourceId === "volc.bigasr.auc";
}
function shouldAttemptVolcengineFlashEndpoint(resourceId, mode) {
    return !shouldUseVolcengineStandardEndpoint(resourceId, mode);
}
function getVolcengineFetchTimeoutMs() {
    const timeoutMs = Number(process.env.VOLCENGINE_FETCH_TIMEOUT_MS);
    if (Number.isFinite(timeoutMs) && timeoutMs >= 5000)
        return Math.floor(timeoutMs);
    return 180000;
}
function getVolcengineFlashMaxLocalBytes() {
    const maxBytes = Number(process.env.VOLCENGINE_FLASH_MAX_LOCAL_BYTES);
    if (Number.isFinite(maxBytes) && maxBytes > 0)
        return Math.floor(maxBytes);
    return 25 * 1024 * 1024;
}
function getVolcengineFlashHardLimitBytes() {
...
function shouldContinueVolcengineStandardPolling(statusCode) {
    const normalized = asText(statusCode);
    return normalized === "20000001" || normalized === "20000002";
}
async function fetchVolcengine(url, init) {
...
async function fetchVolcengineWithRetry(url, init, options) {
...
function normalizeQuickView(input, transcript, durationSeconds) {
    ...
    const normalized = source
        ...
        if (!Number.isFinite(startSec))
            startSec = Math.max(0, index * 300);
        if (!Number.isFinite(endSec) || endSec <= startSec) {
            endSec = Math.min(Math.max(startSec + 90, startSec + 30), Math.max(120, durationSeconds || 0));
        }
        const summary = truncateByChars(item?.summary || item?.text || item?.point, 300);
        if (!summary)
            return null;
        const startTime = formatClock(startSec);
        const endTime = formatClock(endSec);
        return {
            startTime,
            endTime,
...
function normalizeShowNotesKeyMoments(input) {
    const list = Array.isArray(input) ? input : [];
    return list
        .map((item) => ({
...
function getTimedUtteranceSpeakerKey(item, index) {
    const rawSpeaker = asText(item.speaker);
    if (rawSpeaker)
        return rawSpeaker;
    return index === 0 ? "__unknown_lead__" : "__unknown__";
}
function resolveRoleLabels(paragraphs) {
    const firstSeenKeys = [];
    ...
    for (const paragraph of paragraphs) {
        ...
        const hostScoreBoost = (isExplicitHostSpeaker(paragraph.speakerKey) ? 5 : 0) +
    ...
    return paragraphs.map((paragraph, index) => {
        ...
        if (isExplicitHostSpeaker(explicitSpeaker)) {
            // 根据实际名字映射
            if (/ali|阿力/.test(normExplicit))
                return "主播·阿力";
            if (/jessie/.test(normExplicit))
                return "主播·Jessie";
            return hostDisplayName;
        }
        // 嘉宾绝对不能是阿力或 Jessie
...
function titleFromAudioPath(filePath) {
...
function estimateDurationFromBytes(byteLength) {
    if (!Number.isFinite(byteLength) || byteLength <= 0)
        return 0;
    return Math.max(60, Math.min(7200, Math.round(byteLength / 24000)));
}
function normalizeFillerCheckText(value) {
    return asText(value)
        .replace(/[，。！？、,.!?\s~～…]/g, "")
        .toLowerCase();
}
function isFillerOnlyText(value) {
    ...
    if (fillerSet.has(normalized))
        return true;
    if (normalized.length <= 3 && /^([嗯啊哦呃哎诶对是好行])+$/u.test(normalized))
        return true;
    return false;
}
function isLikelyLyricText(value) {
    const raw = asText(value);
    if (!raw)
        return true;
    if (/[♪♫🎵]/u.test(raw))
        return true;
    if (/作词|作曲|编曲|演唱|歌词|副歌|主歌|music/i.test(raw))
        return true;
    const lines = raw
        .split(/\n+/)
        .map((line) => line.trim())
    ...
    if (lines.length >= 3) {
        ...
        if (unique.size <= Math.max(1, Math.floor(lines.length / 2)))
            return true;
    }
    return false;
}
function isMeaningfulText(value) {
    ...
    if (!normalized)
        return false;
    if (isFillerOnlyText(value))
        return false;
    if (isLikelyLyricText(value))
        return false;
    if (normalized.length >= 10)
        return true;
    return /[0-9a-z\u4e00-\u9fa5]{6,}/u.test(normalized);
}
function scoreFeaturedParagraph(text) {
    const normalized = asText(text);
    if (!normalized)
        return -1;
    let score = 0;
    ...
    if (/[。！？!?]/.test(normalized))
        score += 1;
    if (/关键|核心|总结|结论|建议|方法|步骤|重点|模型|原则|框架/.test(normalized))
        score += 3;
    if (/就是|那个|然后|嗯|啊/.test(normalized) && compactLength < 20)
        score -= 2;
    return score;
}
function buildParagraphTranscriptFromTimedItems(items, fallbackDurationSeconds = 180) {
    ...
    for (const item of normalizedItems) {
        ...
        const shouldBreak = gapSeconds > maxGapSeconds ||
    ...
    const finalParagraphs = Number.isFinite(maxParagraphs) && maxParagraphs > 0
...
function splitToTranscriptParagraphs(text, durationSeconds) {
    const normalized = asText(text).replace(/\s+/g, " ");
    if (!normalized)
        return [];
    ...
    const maxParagraphs = Number(process.env.AI_TRANSCRIPT_MAX_PARAGRAPHS || 0);
    const outputParagraphs = Number.isFinite(maxParagraphs) && maxParagraphs > 0
...
function buildHeuristicMetadata(input) {
    ...
    const minutesText = truncateByChars([summaryBody, asText(input.plainText).slice(0, 800)].filter(Boolean).join("\n\n"), 1000);
...
function normalizeMetadataResult(parsed, input, fallbackTitle) {
    ...
    return {
        ...
        summary: {
            ...
            tags: Array.isArray(parsed?.summary?.tags)
                ? parsed.summary.tags.map((item) => asText(item)).filter(Boolean).slice(0, 8)
                : [],
        },
...
function resolveDeepSeekMetadataConfig() {
...
function resolveArkMetadataConfig() {
...
async function extractMetadataWithOpenAiCompatibleProvider(input, fallbackTitle, config) {
    if (!config)
        return null;
...
class OpenAIProgramAiProvider {
    ...
    async transcribeAudio(filePath) {
        ...
        form.append("file", blob, path_1.default.basename(filePath));
        ...
        const transcript = buildParagraphTranscriptFromTimedItems(segments.map((segment, idx) => {
...
class VolcengineProgramAiProvider {
    appId;
    accessToken;
    apiKey;
    resourceIds;
    ...
    detectFormat(filePath) {
        ...
        if (!ext)
            return "wav";
        if (["wav", "mp3", "ogg", "opus", "m4a"].includes(ext))
            return ext;
        return "wav";
    }
    buildHeaders(requestId, resourceId) {
        const headers = {
            "Content-Type": "application/json",
    ...
    async transcribeByStandard(sourceUrl, resourceId, onProgress) {
    ...
    async transcribeAudio(filePath, options) {
        ...
        if (!isLocalSourceUrl && standardResourceIds.length > 0) {
            for (const standardResourceId of standardResourceIds) {
                try {
                    ...
                    if (lowerMessage.includes("invalid audio uri") ||
        ...
        if (isLocalSourceUrl && bytes.length > maxFlashLocalBytes) {
            throw new Error(`火山请求超时风险：本地音频 ${Math.round(bytes.length / 1024 / 1024)}MB 超过 flash 直传建议上限 ${Math.round(maxFlashLocalBytes / 1024 / 1024)}MB；请配置 VOLCENGINE_PUBLIC_BASE_URL 使用公网 URL 转写，或上传更小音频`);
        }
        const payload = {
            user: { uid: this.appId || "podcast-admin" },
            audio: {
        ...
        let json = {};
        const flashFallbackResourceIds = Array.from(new Set([
            ...this.resourceIds.filter((item) => !this.shouldUseStandard(item)),
            "volc.bigasr.auc_turbo",
            "volc.bigasr.auc",
        ]));
        const resourceIdsForRun = (forceFlashFallback || isLocalSourceUrl) && flashFallbackResourceIds.length > 0
            ? flashFallbackResourceIds
            : this.resourceIds;
        if ((forceFlashFallback || isLocalSourceUrl) && bytes.length > flashHardLimitBytes) {
            throw new Error(`当前音频约 ${Math.round(bytes.length / 1024 / 1024)}MB，超出 flash 稳定处理上限 ${Math.round(flashHardLimitBytes / 1024 / 1024)}MB。请在 https://xianfeng.xinzhi.info/admin/programs 上传并使用 standard 模式解析。`);
        }
        ...
        for (const resourceId of resourceIdsForRun) {
            ...
            for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
                ...
                const isTransientGatewayIssue = response.status >= 500 ||
        ...
        const transcript = buildParagraphTranscriptFromTimedItems(utterances.map((item, idx) => {
            const startMs = Number(item?.start_time) || idx * 1000;
            const endMs = Number(item?.end_time) || (startMs + 8000);
            return {
                startSec: Math.max(0, Math.floor(startMs / 1000)),
        ...
        const plainText = asText(json?.result?.text) || transcript.map((item) => item.text).join(" ");
...
function resolveProgramAiProvider() {
...
function resolveVolcengineConfigFromRegistry() {
    try {
        ...
        const enabledAsr = registry.filter((item) => item?.enabled && Array.isArray(item?.capabilities) && item.capabilities.includes("asr"));
        if (enabledAsr.length === 0)
            return {};
        const referencedAsrIds = new Set((Array.isArray(store?.agents) ? store.agents : [])
            .filter((agent) => agent?.status === "active")
            .map((agent) => asText(agent?.feature_models?.asr))
            .filter(Boolean));
        const referencedAsr = enabledAsr.find((item) => referencedAsrIds.has(asText(item?.id)));
        const selected = referencedAsr || enabledAsr[0];
        if (!selected)
            return {};
        const provider = asText(selected.provider).toLowerCase();
        if (!provider.includes("volc") && !provider.includes("doubao") && !provider.includes("byte"))
            return {};
